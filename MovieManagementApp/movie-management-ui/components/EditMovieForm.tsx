...
export default function EditMovieForm({ movieId }: EditMovieFormProps) {
  const [formData, setFormData] = useState<UpdateMovieDto>({
    title: '',
    genre: '',
    rating: undefined,
    posterUrl: ''
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (errors[name as keyof UpdateMovieDto]) {
      setErrors(prev => ({
        ...prev,
        [name]: ''
      }))
  ...
  const validate = (): boolean => {
    const newErrors: Partial<Record<keyof UpdateMovieDto, string>> = {}
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-[400px]">
        <div className="text-2xl text-blue-600 font-semibold">Loading movie...</div>
      </div>
    )
  }
